...
def build_sample_data(n=1200):
    ...
    vendors = [
        "Nusantara Secure Systems", "Arunika Digital", "Sentra Teknologi",
        "Kirana Networks", "Aster Dataworks", "Langit Cyberindo",
        "Prima Integrasi", "Cakra Devices"
    ]
    categories = [
        "Network Security", "Cryptographic Module", "Secure Storage",
        "Authentication Device", "Communication Equipment", "Access Control"
    ]
    modules = [
        "Software Module", "Hardware Module", "Firmware Module",
        "Hybrid Module", "Embedded Module"
    ]
    algorithms = [
        "AES-256", "AES-128", "RSA-2048",
        "SHA-256", "ECC P-256", "HMAC-SHA256"
    ]

    base_start = datetime(2024, 1, 1)
    review_date = datetime(2026, 8, 23)
    rows = []
    certs = []
    ...
    for i in range(1, n + 1):
        ...
        issue_date = base_start + timedelta(days=random.randint(0, 850))
        expiry_date = issue_date + timedelta(days=random.randint(365, 900))
        verify_span = max(20, min(600, max(20, (review_date - issue_date).days)))
        last_verified = issue_date + timedelta(days=random.randint(20, verify_span))

        cert_no = f"CERT-{issue_date.year}-{i:05d}"
        certs.append(cert_no)

        cmvp_status = random.choices(
            ["Verified", "Pending", "Not Found"], weights=[86, 9, 5]
        )[0]
        postel_status = random.choices(
            ["Verified", "Pending", "Not Found"], weights=[84, 10, 6]
        )[0]
        certificate_status = "Active" if expiry_date >= review_date else "Expired"

        roll = random.random()

        if roll < 0.035 and i > 10:
            cert_no = random.choice(certs[:-1])
        elif roll < 0.070:
            cmvp_status = None
        elif roll < 0.105:
            postel_status = None
        elif roll < 0.135:
            expiry_date = review_date - timedelta(days=random.randint(1, 500))
        ...
        product_name = (
            f"{random.choice(['SecureGate','CipherBox','NetShield','AuthKey','DataVault','LinkGuard'])} "
...
def load_sample_data():
    possible_paths = [
        Path("sample_data/certification_review_sample_data.xlsx"),
        Path("certification_review_sample_data.xlsx"),
    ]
